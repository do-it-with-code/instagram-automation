...
def repost_photo(bot, new_media_id, caption, path='posted_medias.txt'):
    if exists_in_posted_medias(new_media_id, path):
        bot.logger.warning("Media {0} was uploaded earlier".format(new_media_id))
        return False
    photo_path = bot.download_photo( new_media_id )
    if not photo_path:
        return False

    if bot.upload_photo(photo_path, caption=caption ):
...
repost_photo(bot, media_id, caption )